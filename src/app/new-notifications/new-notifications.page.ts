...
@Component({
  ...
  styleUrls: ['./new-notifications.page.scss'],
  standalone:false,
})

export class NewNotificationsPage implements OnInit {
  ...
  saveInvo() {
    ...
    this.api.saveNotif(this.newTask).subscribe(data => {
     ...
     if (this.selectedLawyersTeamArr.length > 0) {
      this.selectedLawyersTeamArr.forEach(element => {
        element.notification_id = this.newTask.id
